...
route: /ui/menu
menu: UI
tags: ui, menu, search, groups, menu headings
---
...
Inside `doczrc.js` there is a `themeConfig.menu.search` option to enable or disable search.
...
Inside `doczrc.js` there is a `themeConfig.menu.headings` option.
...
To allow having menus with the same name a `menuDisplayName` option can be set inside `doczrc.js`.
